class UserAddress < ApplicationRecord
  ...
  validates :address, :city, :zip_code, :phone_number, presence: { message: "Поле не може бути порожнім" }
  validate :validate_format

  def validate_format

    if address.present? && !address.match?(/\A(?:пров\.|вул\.|пл\.|просп\.) [а-яА-ЯіІїЇєЄёЁ',\s]+, \d+\z/)
      errors.add(:address, "Неправильний формат адреси")
    end

    if city.present? && contains_digits?(city)
      errors.add(:city, "Місто не повинно містити цифр")
    end

    if city.present? && !in_ukraine?(city)
      errors.add(:city, "Місто повинно бути в Україні")
    end

    if address&.length.to_i > 45
      errors.add(:address, "Адрес дуже довгий (максимум 45 символів)")
    end

    if city&.length.to_i > 25
      errors.add(:city, "Місто дуже довге (максимум 15 символів)")
    end

    if zip_code.present? && zip_code !~ /\A\d{5}\z/
      errors.add(:zip_code, "Поштовий код повинен містити лише цифри")
    end

    if phone_number.present? && (phone_number.gsub(/\s/, '') !~ /^\d{10}$/)
  ...
  def in_ukraine?(city)
    ukrainian_cities = [
      ...
      'Глиняни', 'Глобине', 'Глухів', 'Гнівань', 'Гола Пристань', 'Голубівка', 'Горішні Плавні', 'Горлівка', 'Городенка',
      'Городище', 'Городня', 'Городок', 'Горохів', 'Гребінка', 'Гуляйполе', 'Дебальцеве', 'Деражня', 'Дергачі', 'Джанкой', 'Дніпро', 'Дніпрорудне',
      'Добромиль', 'Добропілля', 'Довжанськ', 'Докучаєвськ', 'Долина', 'Долинська', 'Донецьк', 'Дрогобич', 'Дружба',
      'Дружківка', 'Дубляни', 'Дубно', 'Дубровиця', 'Дунаївці', 'Енергодар', 'Євпаторія', 'Єнакієве', 'Жашків', 'Жданівка',
      'Жидачів', 'Житомир', 'Жмеринка', 'Жовква', 'Жовті Води', 'Заводське', 'Залізне', 'Заліщики', 'Запоріжжя', 'Заставна',
      'Збараж', 'Зборів', 'Звенигородка', 'Звягель', 'Здолбунів', 'Зеленодольськ', 'Зимогір\'я', 'Зіньків', 'Зміїв', 'Знам\'янка',
      'Золоте', 'Золотоноша', 'Золочів', 'Зоринськ', 'Зугрес', 'Івано-Франківськ',
      ...
      'Могилів-Подільський', 'Молодогвардійськ', 'Молочанськ', 'Монастириська', 'Монастирище', 'Моршин', 'Моспине', 'Мостиська',
      'Мукачево', 'Надвірна', 'Немирів', 'Нетішин', 'Ніжин', 'Нікополь', 'Нова Каховка', 'Нова Одеса',
      'Новгород-Сіверський', 'Новий Буг', 'Новий Калинів', 'Новий Розділ', 'Новоазовськ', 'Нововолинськ', 'Новогродівка', 'Новодністровськ',
      'Новодружеськ', 'Новомиргород', 'Новомосковськ', 'Новоселиця', 'Новоукраїнка', 'Новояворівськ', 'Носівка', 'Обухів',
      'Овруч', 'Одеса', 'Олевськ', 'Олександрівськ', 'Олександрія', 'Олешки', 'Оріхів', 'Остер', 'Острог',
      'Охтирка', 'Очаків', 'Павлоград', 'Первомайськ', 'Первомайськ', 'Первомайський', 'Перевальськ', 'Перемишляни',
      'Перечин', 'Перещепине', 'Переяслав', 'Першотравенськ', 'Петрово-Красносілля', 'Пирятин', 'Південне', 'Підгайці',
      ...
      'Святогірськ', 'Севастополь', 'Селидове', 'Семенівка', 'Середина-Буда', 'Сєвєродонецьк', 'Синельникове', 'Сіверськ', 'Сімферополь',
      'Скадовськ', 'Скалат', 'Сквира', 'Сколе', 'Славута', 'Славутич', 'Слов\'янськ', 'Сміла', 'Снігурівка',
      'Сніжне', 'Сновськ', 'Снятин', 'Сокаль', 'Сокиряни', 'Соледар', 'Сорокине', 'Соснівка', 'Старий Крим',
      'Старий Самбір', 'Старобільськ', 'Старокостянтинів', 'Стебник', 'Сторожинець', 'Стрий', 'Судак', 'Судова Вишня',
      'Суми', 'Суходільськ', 'Таврійськ', 'Тальне', 'Тараща', 'Татарбунари', 'Теплодар', 'Теребовля', 'Тернівка',
      ...
      'Южне', 'Южноукраїнськ', 'Южноукраїнськ', 'Яворів', 'Яготин', 'Ялта', 'Ямпіль', 'Ямпіль (Вінницька область)', 'Яремче',
      'Ясинувата'
    ]
